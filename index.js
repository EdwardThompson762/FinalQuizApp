...
function questionPage (correctAnswers, question, questionsAnswered){
  ...
  <section id="questionsPage">
    ...
    <div id="questionDiv">
      ...
      <span id="currentScore">Score: ${correctAnswers}/${questionsAnswered}</span>
    </div>
</section>
  `;
} 
...
function theRestartButton() {
  $('#quizForm').on('click', '#restart button', function(event) {
    questionNum = 1;
    correctAnswers = 0;
    nextQuestion();
  });
...
function wrongAwnser() {
  $('#quizForm').html(incorrectFeedback(questionNum));
}

function incorrectFeedback() {
  return `
    <section class="feedbackPage" role="main">
      <h2>Study harder Witcher! It was ${ANSWERS[questionNum -1]}!</h2>
...
function  resultsPage(correctAnswers) {
  let finalMessage = ""
    if (correctAnswers <= 6)
    finalMessage = "Study harder Witcher!"
    else
    finalMessage = "Well done Witcher"
  $('#quizForm').html(`
    <section id="lastPage">
      <h1>${finalMessage}</h1>
      <h2>Final Score: 
      ${correctAnswers} out of 10</h2>
